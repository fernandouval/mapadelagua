...
var topogramAgua = function topogramAgua(options) {
    ...
    self.init = function () {
        ...
        self.zoom = d3.zoom().scaleExtent([1, 8]).on("zoom", function () {
        ...
        d3.select("#btn-zoom-in").on('click', function () {
            ...
            self.svg.transition().duration(100).call(self.zoom.scaleTo, self.zoom_value);
        });
        // Zoom out
    ...
    self.renderDepartamentos = function (departamentos) {
        ...
        self.g2.selectAll("path").data(self.geodata_deps.features).enter().append("path").attr("fill", self.custom_color['default']).style("opacity", 0.5).style("pointer-events", "none").attr("d", self.path);
    };
    self.renderBase = function (zones) {
        self.geodata_base = topojson.feature(zones, zones.objects.disponibilidad01);

        self.g2.selectAll("path").data(self.geodata_base.features).enter().append("path").attr("fill", function (d) {
            return self.custom_color[d.properties.classes];
        }).style("opacity", 1).style("pointer-events", "none").attr("d", self.path);
    };
    ...
    self.prerenderCuencas = function (topo) {
        ...
        self.states = self.states.data(self.cartogram.features(self.topology, self.geometries)).enter().append("path")
        //.attr("fill", self.custom_color['Media'])
        .style("opacity", 0.5).attr("d", self.path).on('mousemove', function (d) {
            self.showTooltip(d);
        ...
        $("#check").click(function () {
            ...
            f = true;
            $(this).toggleClass('on');
            var key = $(this).hasClass('on') ? 'resoluciones' : 'area';
    ...
    self.updateSearch = function (licencias) {
    ...
    self.updateResults = function () {
        ...
        if (init) {
            ...
            self.g2.selectAll(".mark").data(marks).enter().append("image").attr('class', 'mark').attr('width', 10).attr('height', 10).attr("xlink:href", 'https://cdn3.iconfinder.com/data/icons/softwaredemo/PNG/24x24/DrawingPin1_Blue.png').attr("transform", function (d) {
                return "translate(" + self.projection(d) + ")";
            });
    ...
    self.showInfo = function (d) {
        ...
        if (self.data.cuencas[parseInt(d.properties.CODIGO)] !== undefined) {
            ...
            var _cuenca_map = new cuencaMap({
                parent_id: 'cuenca-map',
                width: $("#cuenca-map").width(),
                height: $("#cuenca-map").height()
            });
            _cuenca_map.render(d, self.geodata_deps, self.projection, self.new_colors[self.priorizacion[parseInt(d.properties.CODIGO)]]);
        }
    };
    ...
    self.renderCuencas = function (key) {
        ...
        var value = function value(d) {
            if (key == 'area') {
                ...
                if (self.data.cuencas[parseInt(d.properties.CODIGO)] === undefined || self.filter_dh[self.data.cuencas[parseInt(d.properties.CODIGO)].DH] === false) return 0;
